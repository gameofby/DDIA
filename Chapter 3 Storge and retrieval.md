...
如图，列存中有一个关键点，列中对应的行顺序(index)要一致
...
详细见图：
将一列原始数据，重新编码为多行distinct bitmap. 每个bitmap代表列中一个唯一的数字，bitmap中的每一位代表一个row. 如果(row, column)的值和bitmap代表的distinct value相同, 则bitmap对应的digit为1，否则为0
